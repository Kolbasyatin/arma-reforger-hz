...
Отдельный от SteamKit канал: обычный публичный API Valve по ключу с `steamcommunity.com/dev/apikey`
(требует мобильный аутентификатор на аккаунте, 100 000 вызовов в сутки). К билету для Bohemia
отношения не имеет.

Эндпоинт `GET /steam/{steamId64}` собирает четыре источника параллельно и отдаёт сырые ответы
как есть, ничего не сохраняя: хранение — забота armaplayers.

Ключ НЕ хранится в appsettings. Только переменная окружения `SteamWebApi__ApiKey`;
без неё эндпоинт отвечает 503.

Наблюдения по реальным ответам:

- `GetOwnedGames` у закрытых игровых деталей отдаёт HTTP 200 и `{"response":{}}` — пустой ответ,
  а не ошибку. Судить об успехе по коду ответа здесь нельзя.
- `GetPlayerBans` отвечает в PascalCase (`SteamId`, `VACBanned`), остальные методы — в snake_case.
- `playtime_2weeks` меняется у активного игрока постоянно, поэтому дедупликация снимков
  по хешу должна быть ПО ИСТОЧНИКАМ: иначе меняющиеся игры будут переписывать и профиль.
